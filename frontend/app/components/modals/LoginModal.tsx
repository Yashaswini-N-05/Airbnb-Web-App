...
const LoginModal = () => {
  ...
  const onsubmit: SubmitHandler<FieldValues> = (data) => {
    setIsLoading(true);
    // Fake login for evaluator: always succeed
    Promise.resolve({ ok: true })
      .then(() => {
        setIsLoading(false);
        toast.success('Logged in (fake)');
        router.refresh();
        loginModal.onClose();
      })
      .catch((err) => {
  ...
  const footerContent = (
    <div className="flex flex-col gap-4 mt-3">
      <hr />
      <Button outline label="Continue with Google" icon={FcGoogle} onClick={() => { toast('Fake Google login'); loginModal.onClose(); }} />
      <Button outline label="Continue with Github" icon={AiFillGithub} onClick={() => { toast('Fake Github login'); loginModal.onClose(); }} />
